...
MONERO_DIR=charnacoin
MONEROD_EXEC=charnacoind
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/charnacoin-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MONEROD_EXEC=charnacoind.exe
fi
...
cd build
qmake ../charnacoin-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy charnacoind to bin folder
